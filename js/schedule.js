class Schedule {
    ...
    addClass(section) {
        ...
        let lecture_types = ["Lecture", "Studio", "Clinical", "Dummy Course", "Externship", "Independent Study/Research", "Internship", "Seminar", "Workshop"];
        ...
        if (labs.length > 0) {
            let newerSchedules = newSchedules.map(ns => {
                let labSchedules = [];
                let sameTeacherLabs = labs.filter(l => l.instructors[0] === ns.classes.find(c => c.course === l.course).instructors[0]);
    ...
    clone() {
        let clonedClasses = $.extend(true, [], this.classes);
        let clonedFreetimes = $.extend(true, {}, this.freetimes);
        let clonedTeachers = $.extend(true, {}, this.teachers);
